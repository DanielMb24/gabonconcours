...
class CandidatureStateManager {
    ...
    async initializeNewCandidature(concoursId: string): Promise<CandidatureState> {
        try {
            const concoursResponse = await fetch(`https://gabonconcours-1.onrender.com/api/concours/${concoursId}`);
    ...
    async initializeContinueCandidature(nupcan: string): Promise<any> {
        try {
            const response = await fetch(`https://gabonconcours-1.onrender.com/api/candidats/nupcan/${encodeURIComponent(nupcan)}`);
    ...
    async updateProgression(nupcan: string, etape: 'documents' | 'paiement'): Promise<void> {
        try {
            ...
            await fetch(`https://gabonconcours-1.onrender.com/api/candidats/${nupcan}/progression`, {
                method: 'PUT',
                headers: {
